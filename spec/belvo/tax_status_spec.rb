...
RSpec.describe Belvo::TaxStatus do
  let(:tax_status_resp) do
    {
      id: 'some-tax-status-uuid',
      link: 'some-link-uuid',
  ...
  def mock_create_ok
    mock_login_ok
    WebMock.stub_request(:post, 'http://fake.api/api/tax-status/').with(
  ...
  it 'can create' do
    ...
    ).to eq(tax_status_resp.transform_keys(&:to_s))
  end
